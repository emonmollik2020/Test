...
# ওয়েবসেকট সিঙ্ক ট্র্যাকিং ভ্যারিয়েবল
...
async_exchange = ccxtpro.bitget({'enableRateLimit': True})
...
# =====================================================================
# SECTION 2: ফাইল ও ডাটাবেস ম্যানেজমেন্ট
# =====================================================================
def clean_float(val):
...
# =====================================================================
# SECTION 3: ক্রিপ্টো-অপ্টিমাইজড ক্যান্ডেলস্টিক প্যাটার্ন ডিটেক্টর
# =====================================================================
def get_advanced_pats(df):
    ...
    if l1 >= 2.0 * b1 and u1 <= 0.2 * t1: 
        p.append({"n": "হ্যামার 🔨", "t": "bull"})
    if u1 >= 2.0 * b1 and l1 <= 0.2 * t1: 
        p.append({"n": "শুটিং স্টার ☄️", "t": "bear"})
        
    if not g2 and g1 and c1['c'] > c2['o']:
        p.append({"n": "বুলিশ এনгалফিং 📈", "t": "bull"})
    if g2 and not g1 and c1['c'] < c2['o']:
        p.append({"n": "বেয়ারিশ এনгалফিং 📉", "t": "bear"})

    if (b1 / t1) < 0.15:
        p.append({"n": "দোজি ✝️", "t": "bull"})
    ...
    if df is None or df.empty:
        ...
        if all_candles:
            ...
            if db_enabled:
                try:
                    df.to_sql('sol_15m_history', engine, if_exists='append', index=False)
                    print("Saved bootstrapped candles to Supabase Cloud.")
                except Exception as e:
...
# =====================================================================
# SECTION 4: ওয়েবসেকট ক্যান্ডেল ডেটা সিঙ্ক্রোনাইজেশন (WebSocket Background Tasks)
# =====================================================================
def merge_websocket_candles_to_global(new_candles_list):
    global GLOBAL_DF
    if not new_candles_list:
        return
    df_new = pd.DataFrame(new_candles_list, columns=['t', 'o', 'h', 'l', 'c', 'v'])
    with GLOBAL_DF_LOCK:
        if GLOBAL_DF.empty:
            GLOBAL_DF = df_new
        else:
            combined = pd.concat([GLOBAL_DF, df_new])
            combined = combined.drop_duplicates(subset=['t'], keep='last').sort_values('t').reset_index(drop=True)
            if len(combined) > MAX_CANDLES_TO_KEEP:
                combined = combined.iloc[-MAX_CANDLES_TO_KEEP:]
            GLOBAL_DF = combined.copy()


async def sol_websocket_task():
    global LAST_WS_UPDATE_TIME
    print("Starting SOL/USDT 15m WebSocket Stream...")
    while True:
        try:
            candles = await async_exchange.watch_ohlcv(SYMBOL, '15m')
            if candles:
                merge_websocket_candles_to_global(candles)
                LAST_WS_UPDATE_TIME = time.time() # সফল আপডেটের সময় রেকর্ড
        except Exception as e:
            print(f"SOL WS Stream Exception: {e}. Reconnecting in 5 seconds...")
            await asyncio.sleep(5)


async def btc_websocket_task():
    global CANDLE_CACHE_BTC, LAST_WS_UPDATE_TIME
    print("Starting BTC/USDT 15m WebSocket Stream...")
    while True:
        try:
            candles = await async_exchange.watch_ohlcv("BTC/USDT:USDT", '15m')
            if candles:
                CANDLE_CACHE_BTC = candles
                LAST_WS_UPDATE_TIME = time.time()
        except Exception as e:
            print(f"BTC WS Stream Exception: {e}. Reconnecting in 5 seconds...")
            await asyncio.sleep(5)


# =====================================================================
# SECTION 4.5: আসিঙ্ক্রোনাস ট্রেডিং বট ইঞ্জিন (WebSocket-Fed Engine Loop)
# =====================================================================
async def async_bot_engine():
    global CANDLE_CACHE_BTC, GLOBAL_DF, LAST_WS_UPDATE_TIME
    
    await asyncio.to_thread(init_db_tables)
    
    cur_init = await asyncio.to_thread(load_state)
    ...
    print("Initializing SOL 15m Database (Async Block)...")
    await asyncio.to_thread(bootstrap_or_backfill_sol)

    asyncio.create_task(sol_websocket_task())
    asyncio.create_task(btc_websocket_task())
    ...
    while True:
        try:
            # -----------------------------------------------------------------
            # [হাইব্রিড মেকানিজম]: ওয়েবসেকট ডেটা ড্রপ করলে বা ব্লক থাকলে REST এপিআই দিয়ে ব্যাকআপ আনা
            # -----------------------------------------------------------------
            is_ws_active = (time.time() - LAST_WS_UPDATE_TIME) < 30.0 # গত ৩০ সেকেন্ডে আপডেট এসেছে কিনা
            
            if not is_ws_active:
                # ওয়েবসেকট কাজ না করলে ব্যাকআপ হিসেবে REST API দিয়ে লাইভ ডেটা আনা হবে
                try:
                    # নন-ব্লকিং ব্যাকগ্রাউন্ড থ্রেডে REST কল করা
                    raw_sol = await asyncio.to_thread(fetch_ohlcv_strict, SYMBOL, '15m', None, 5)
                    if raw_sol:
                        merge_websocket_candles_to_global(raw_sol)
                        
                    raw_btc = await asyncio.to_thread(fetch_ohlcv_strict, "BTC/USDT:USDT", '15m', None, 5)
                    if raw_btc:
                        CANDLE_CACHE_BTC = raw_btc
                        
                    # ব্যাকআপ মোড অ্যাক্টিভ হওয়ার মেসেজ
                    wait_reason_prefix = "রানিং (REST ফলব্যাক মোড ⚠️)"
                except Exception as rest_err:
                    print(f"REST Fallback Error: {rest_err}")
                    wait_reason_prefix = "সংযোগ ত্রুটি (পুনরায় চেষ্টা চলছে)"
            else:
                wait_reason_prefix = "রানিং (ওয়েবসকেট মোড ⚡)"
            ...
            if df15.empty:
                cur = await asyncio.to_thread(load_state)
                cur["wait_reason"] = "ডেটা সিঙ্ক হচ্ছে... ⏳"
                await asyncio.to_thread(save_state, cur)
                await asyncio.sleep(5)
                continue
            
            df_btc = pd.DataFrame(CANDLE_CACHE_BTC, columns=['t', 'o', 'h', 'l', 'c', 'v']) if CANDLE_CACHE_BTC else pd.DataFrame()
            if df_btc.empty:
            ...
            if in_pos:
                ...
                cur["wait_reason"] = f"কুলডাউন ({int(remaining_seconds/60)} মিনিট বাকি) - {wait_reason_prefix}"
            elif not btc_bullish and p > e200:
                cur["wait_reason"] = f"বিটকয়েন ট্রেন্ড ডাউন (BTC Bearish) - {wait_reason_prefix}"
            elif btc_bullish and p < e200:
                cur["wait_reason"] = f"বিটকয়েন ট্রেন্ড আপ (SOL SHORT এর উপযুক্ত নয়) - {wait_reason_prefix}"
            elif not vwap_long_confirmed and p > e200:
                cur["wait_reason"] = f"মূল্য VWAP লাইনের নিচে (Bearish Volume Zone) - {wait_reason_prefix}"
            elif not vwap_short_confirmed and p < e200:
                cur["wait_reason"] = f"মূল্য VWAP লাইনের ওপরে (Bullish Volume Zone) - {wait_reason_prefix}"
            elif not volume_confirmed:
                cur["wait_reason"] = f"দুর্বল ভলিউম (ভলিউম ব্রেকআউটের অপেক্ষা) - {wait_reason_prefix}"
            elif not ema_long_alignment and p > e200:
                cur["wait_reason"] = f"১৫-মিনিট চার্টে ল্যাপ বা রিট্রেসমেন্ট চলছে - {wait_reason_prefix}"
            elif not ema_short_alignment and p < e200:
                cur["wait_reason"] = f"১৫-মিনিট চার্টে বাউন্স ব্যাক বা কারেকশন চলছে - {wait_reason_prefix}"
            else:
                cur["wait_reason"] = f"সুইং এন্ট্রি প্যাটার্ন খুঁজছে... - {wait_reason_prefix}"
                
            await asyncio.to_thread(save_state, cur)
            
        except Exception as e:
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Async Bot Engine Warning: {e}")
            
        await asyncio.sleep(10)


def start_asyncio_loop():
    """ইভেন্ট লুপের কোনো অব্যাখ্যাত ক্র্যাশ ট্র্যাকিং করার জন্য হ্যান্ডলার"""
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(async_bot_engine())
    except Exception as e:
        import traceback
        error_msg = f"Thread Crash: {str(e)}\n{traceback.format_exc()}"
        print(error_msg)
        try:
            cur = load_state()
            cur["wait_reason"] = f"ক্র্যাশ ত্রুটি: {str(e)[:40]} 🛑"
            save_state(cur)
        except Exception:
            pass
...
# =====================================================================
# SECTION 5: Flask ওয়েব সার্ভার এবং এপিআই রাউটস (অপরিবর্তিত)
# =====================================================================
@app.route('/api/data')
def api():
...
def api_ohlcv():
    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            tf_sup = 0.0
            if len(df_res) >= 21:
                tf_res = float(df_res['h'].iloc[-21:-1].max())
                tf_sup = float(df_res['l'].iloc[-21:-1].min())
            elif len(df_res) >= 2:
                tf_res = float(df_res['h'].iloc[:-1].max())
                tf_sup = float(df_res['l'].iloc[:-1].min())
            
            df_limit = df_res.tail(200)
            candles = []
            for _, row in df_limit.iterrows():
                t_val = int(row['t'])
                time_seconds = int(t_val / 1000) if t_val > 9999999999 else t_val
                candles.append({
                    "time": time_seconds,
                    "open": float(row['o']),
                    "high": float(row['h']),
        ...
        print(f"Error serving ohlcv data: {e}")
    return jsonify({"candles": [], "tf_res": 0.0, "tf_sup": 0.0, "tf_name": ""})
...
# =====================================================================
# SECTION 6: ড্যাশবোর্ড UI টেমপ্লেট (অপরিবর্তিত)
# =====================================================================
UI = """
<!DOCTYPE html>
<html lang="bn">
...
<script>
    ...
    async function loadCandles() {
        try {
            ...
            if (data && data.length > 0) {
                ...
                chart.timeScale().fitContent(); 
                updateMarkersAndLines(res.tf_res, res.tf_sup, res.tf_name);
            }
        } catch (e) {
    ...
    function updateMarkersAndLines(tf_res, tf_sup, tf_name) {
        ...
        if (d.pdh > 0) {
            active_price_lines.push(candleSeries.createPriceLine({
                price: d.pdh, color: '#f87171', lineWidth: 1, lineStyle: 2, axisLabelVisible: true, title: 'Daily High (PDH)'
            }));
        }
        if (d.pdl > 0) {
            active_price_lines.push(candleSeries.createPriceLine({
                price: d.pdl, color: '#34d399', lineWidth: 1, lineStyle: 2, axisLabelVisible: true, title: 'Daily Low (PDL)'
            }));
        }
    ...
    async function update() {
        try {
            ...
            if (d.price > 0) {
                ...
                renderActiveTF();
                updateMarkersAndLines(global_data.tf_res, global_data.tf_sup, global_data.tf_name);
