...
# ডিফল্ট স্টেট (সবগুলো টাইমফ্রেমের জন্য অ্যানালাইসিস অবজেক্ট যুক্ত করা হয়েছে)
DEFAULT_STATE = {
    "price": 0.0,
    ...
    # নতুন সকল টাইমফ্রেমের অ্যানালাইসিস স্কিমা
...
def resample_tf(df_temp, tf_str):
    """
    Pandas-এর সাহায্যে লোড হওয়া ১৫-মিনিটের ক্যান্ডেলগুলো যেকোনো টাইমফ্রেমে রিস্যাম্পল করবে।
    """
...
def analyze_tf(df_res, original_price):
    """
    যেকোনো রিস্যাম্পল্ড ক্যান্ডেল ডেটাফ্রেমের জন্য RSI, EMA ২০ এবং EMA ৫০ গণনা করবে।
    ডাটার পরিমাণ কম থাকলেও এটি ডায়নামিক উইন্ডো দিয়ে নিরাপদে হিসাব সম্পন্ন করে।
    """
    if len(df_res) < 2:
...
def bot_engine():
    ...
    while True:
        try:
            ...
            # ১ ঘণ্টা কাস্টম অ্যানালাইসিস (BTC Price ও EMA 200 এর জন্য ব্যাকওয়ার্ড কমপ্যাটিবিলিটি রাখা হয়েছে)
            ...
            cur.update({
                ...
                # ক্লাউড ও মেমোরিতে সব আপডেট করা হচ্ছে
...
# =====================================================================
# SECTION 6: ড্যাশবোর্ড UI টেমপ্লেট (প্রিমিয়াম ট্যাব সহ ডিজাইন)
# =====================================================================
UI = """
<!DOCTYPE html>
<html lang="bn">
<head>
    ...
    <style>
        ...
        /* কাস্টম স্ক্রোলবার হাইড করার স্টাইল (মোবাইল ডিভাইসে হরাইজন্টাল ট্যাবের জন্য) */
...
<div class="max-w-md mx-auto">
    ...
    <div class="card p-4 mb-4 text-[11px]">
        ...
        <!-- হরাইজন্টালি স্ক্রোল করা যাবে মোবাইল স্ক্রিনে সহজে দেখার জন্য -->
        ...
        <!-- ১ ঘণ্টা সিলেকশনের এক্সট্রা কাস্টম রো (EMA 200 ও BTC Price) -->
...
<script>
    let active_tf = '15m'; // ডিফল্ট সিলেকশন
    let global_data = null;

    // টাইমফ্রেম বাটন ক্লিকের মেথড
    function selectTF(tf) {
        active_tf = tf;
        renderActiveTF();
    }

    // অ্যাক্টিভ টাইমফ্রেমের ডেটা রেন্ডার করার লজিক
    function renderActiveTF() {
        if (!global_data) return;
        const d = global_data;
        
        // বাটনগুলোর অ্যাক্টিভ স্টেট টগল করা
        ...
        // নির্দিষ্ট টাইমফ্রেমের ডেটা নিয়ে আসা
        ...
        // VWAP শুধুমাত্র ১৫মিনিট টাইমফ্রেমে দেখাবে
        ...
        // EMA 200 এবং BTC Price শুধুমাত্র ১ ঘণ্টার টাইমফ্রেমে দেখাবে
        ...
        // ক্যান্ডেলস্টিক প্যাটার্ন রেন্ডারিং
    ...
    async function update() {
        try {
            ...
            if (d.price > 0) {
                global_data = d; // লোকাল ভ্যারিয়েবলে ক্যাশ সংরক্ষণ করা হলো
                ...
                // অ্যাক্টিভ সিলেক্টেড ট্যাবটি রেন্ডার করা
                renderActiveTF();
