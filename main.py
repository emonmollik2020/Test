...
# =====================================================================
# সিকিউরিটির জন্য ডাটাবেস ইউআরএল রেন্ডার এনভায়রনমেন্ট ভ্যারিয়েবল থেকে নেওয়া হচ্ছে
...
def clean_float(val):
    """
    জেসন ওভারফ্লো বা NaN এরর এড়াতে ফ্লোট মান পরিশোধন করার ফাংশন
    """
...
def init_db_tables():
    """
    Supabase-এ স্বয়ংক্রিয়ভাবে ক্যান্ডেল ও স্টেট সংরক্ষণের টেবিল তৈরি করবে।
    """
    if not db_enabled:
        return
    try:
        with engine.connect() as conn:
            # ক্যান্ডেল হিস্ট্রি টেবিল তৈরি
            conn.execute(text("""
            ...
            """))
            # বটের স্টেট ও ব্যালেন্স ট্র্যাকিং টেবিল তৈরি
            conn.execute(text("""
...
def save_state(d):
    """
    লোকাল ড্যাশবোর্ড আপডেট করতে ফাইল এবং দীর্ঘমেয়াদী সুরক্ষায় ক্লাউড ডাটাবেসে সেভ করবে।
    """
    with STATE_LOCK:
        # ১. লোকাল ক্যাশে দ্রুত সেভ (ড্যাশবোর্ড ফিডের জন্য)
        try:
...
def load_state():
    """
    বট রিস্টার্ট হলে ডাটাবেস বা লোকাল ফাইল থেকে রিকভারি করবে।
    """
    ...
    with STATE_LOCK:
        ...
        # প্রথম বুট বা রিস্টার্টের সময় ক্লাউড থেকে রিকভার করা
        if db_enabled:
            try:
                with engine.connect() as conn:
...
def fetch_ohlcv_strict(symbol, timeframe, since=None, limit=None):
    """
    কঠোরভাবে শুধুমাত্র বিটগেট (Bitget) এক্সচেঞ্জ থেকেই ওএইচএলসিভি ডেটা সংগ্রহ করবে।
    """
...
def bootstrap_or_backfill_sol():
    """
    ২.৫ মিনিট সময় নিয়ে ডেটা ক্লাউড ডেটাবেস (Supabase) বা লোকাল ডিস্ক থেকে লোড করবে।
    """
    now_ms = int(time.time() * 1000)
    df = None
    
    # ১. প্রথমে ক্লাউড ডাটাবেস থেকে লোড করার চেষ্টা
    if db_enabled:
        try:
    ...
    # ২. লোকাল ফাইল ব্যাকআপ চেক (যদি ডাটাবেস সক্রিয় না থাকে)
    ...
    if df is None or df.empty:
        ...
        for step in range(1, total_steps + 1):
            ...
            try:
                # শুধুমাত্র বিটগেট থেকে ওএইচএলসিভি ফেচ করা
        ...
        if all_candles:
            ...
            df.to_csv(HISTORY_FILE, index=False) # লোকাল ব্যাকআপ সেভ
    ...
    # ৪. ব্যাকফিল লজিক (গ্যাপ থাকলে সিঙ্ক)
    else:
...
def bot_engine():
    ...
    # ক্লাউড ডাটাবেস টেবিল ইনিশিয়েলাইজেশন
    init_db_tables()
    ...
    # ক্লাউড থেকে ডাটাবেস লোড ও সিঙ্ক করা
    print("Initializing SOL 15m Database...")
    df_sol = bootstrap_or_backfill_sol()

    while True:
        try:
            # --- ইনক্রিমেন্টাল ক্লাউড ক্যাশিং লজিক (SOL - প্রতি লুপে ২ ক্যান্ডেল) ---
            try:
                raw_sol = fetch_ohlcv_strict(SYMBOL, '15m', limit=2)
                df_new = pd.DataFrame(raw_sol, columns=['t', 'o', 'h', 'l', 'c', 'v'])
                
                # PostgreSQL প্রাইমারি কি ভায়োলেশন এড়াতে ডুপ্লিকেট টাইমস্ট্যাম্পগুলো ডিলিট করে ইনসার্ট করা (UPSERT)
                if db_enabled:
                    with engine.connect() as conn:
            ...
            # ডেটাবেস ফাস্ট টাইম খালি থাকলে সেফটি গার্ড
            if df15.empty:
                cur = load_state()
            ...
            # S&R লেভেল এক্সট্রাক্ট করা (জেসন এরর এড়াতে মানগুলো পরিশোধন করা হয়েছে)
            ...
            # সিনট্যাক্স এরর সংশোধন (&& এর পরিবর্তে correct python operator: and)
            ...
            # ড্যাশবোর্ডের জন্য ওয়েटिंग মেসেজ সাজানো
            if in_pos:
