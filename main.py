...
def load_state():
    if not os.path.exists(STATE_FILE):
        return {"price":0.0,"balance":100.0,"total_pnl":0.0,"last_update":"...","trades":0,"win_rate":0,"best":0.0,"worst":0.0,"last_action":"---","in_position":False,"live_pnl_pct":0.0,"live_pnl_val":0.0,"entry_price":0.0,"sl_level":0.0,"tp_level":0.0,"analysis_1m":{"rsi":0,"ema":0,"sig":"অপেক্ষা","pats":[]},"analysis_3m":{"rsi":0,"macd":0,"sig":"অপেক্ষা","pats":[]},"wait_reason":"লোড হচ্ছে...","log":[],"history":[]}
...
# --- ১৬টি প্যাটার্ন ডিটেক্টর (বাংলা নাম সহ) ---
def get_advanced_pats(df):
    ...
    if not g2 and g1 and c1['o'] < c2['c'] and c1['c'] > (c2['o']+c2['c'])/2: p.append({"n": "পিয়ার্সিং লাইন", "t": "bull"})
...
def bot_engine():
    ...
    pnl_hist, in_pos, entry_p, peak_p = [0], False, 0.0, 0.0
    while True:
        try:
            ...
            live_val = (100.0/entry_p*p)-100.0 if in_pos else 0.0

            if in_pos and p > peak_p:
                peak_p = p
                cur.update({"sl_level": round(p*(1-DEF_SL),2), "tp_level": round(p*(1+DEF_TP),2)})

            cur.update({"price":round(p,2),"last_update":datetime.now(timezone.utc).strftime("%H:%M:%S"),"in_position":in_pos,"live_pnl_pct":round(live_pnl,2),"live_pnl_val":round(live_val,2),"entry_price":round(entry_p,2),"analysis_1m":{"rsi":round(r1,1),"ema":round(e20,2),"sig":"বুলিশ ✅" if p>e20 else "বেয়ারিশ ❌","pats":get_advanced_pats(df1)},"analysis_3m":{"rsi":round(r3,1),"macd":round(mv,3),"sig":"বুলিশ ✅" if mv>ms else "অপেক্ষা","pats":get_advanced_pats(df3)}})
            ...
            cur["wait_reason"] = "ট্রেড লাইভ আছে (পজিশনে)" if in_pos else ("১মি ট্রেন্ড দুর্বল" if p<=e20 else "এন্ট্রি খুঁজছে...")
            save_state(cur)
        except: pass
        time.sleep(10)
...
<head>
    ...
    <style>
        ...
        .tag { border: 1px solid #dcfce7; color: #166534; padding: 2px 8px; border-radius: 99px; font-size: 10px; font-weight: 700; display: inline-block; margin: 2px; }
...
<div class="max-w-md mx-auto">
    ...
    <div class="card p-6 mb-4 text-center">
        <div class="flex justify-between items-center mb-4"><span id="pr" class="text-4xl font-black tracking-tighter">$0.00</span><div class="text-right text-[10px] text-slate-400">ব্যালেন্স: <b id="bl">$100.00</b></div></div>
        ...
        try {
            ...
            if(d.price > 0) {
                ...
                if(d.in_position) {
                    ...
                    disp.className = 'mb-4 p-5 border-2 rounded-3xl text-center bg-white shadow-lg ' + bor;
