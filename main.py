            ...
            try:
                with engine.connect() as conn:
                    ...
                    if result:
                        ...
                        # পুরোনো ডাটা লোডের সময় যাতে নতুন কিগুলো মিসিং না হয় তার সেফটি মার্জার
...
def get_advanced_pats(df):
    """
    ক্রিপ্টো ফিউচার্স মার্কেটের ডাইনামিক কন্ডিশন অনুযায়ী অপ্টিমাইজড প্যাটার্ন ডিটেক্টর।
    """
    p = []
    if len(df) < 3:
        return p
...
def api_ohlcv():
    """
    ডাইনামিক টাইমফ্রেম অনুযায়ী ক্যান্ডেল ও চার্ট লেভেলের S&R হিসেব করার অত্যন্ত স্মার্ট মেথড।
    """
    ...
    try:
        if db_enabled:
            # সরাসরি ক্লাউড ডাটাবেজ থেকে হিস্ট্রি রিড করা হচ্ছে
            df = pd.read_sql("SELECT * FROM sol_15m_history ORDER BY t ASC", engine)
            if df is not None and not df.empty:
                # ডাইনামিক রিস্যাম্পলিং (সিলেক্টেড টাইমফ্রেম অনুযায়ী)
                ...
                # ক্র্যাশ এড়াতে নিশ্চিত সোর্টিং লজিক
                df_res = df_res.dropna(subset=['t','o','h','l','c']).sort_values('t')
                
                # সিলেক্টেড টাইমফ্রেমের ডাইনামিক সাপোর্ট-রেজিস্ট্যান্স হিসাব করা (গত ২০টি মোমবাতির সর্বোচ্চ ও সর্বনিম্ন)
                tf_res = 0.0
                tf_sup = 0.0
                if len(df_res) >= 21:
                    # রানিং অসম্পূর্ণ ক্যান্ডেল (iloc[-1]) বাদ দিয়ে পূর্বের ২০টি ক্যান্ডেল থেকে হিসাব
                ...
                df_limit = df_res.tail(200) # শেষ ২০০ ক্যান্ডেল নেওয়া হচ্ছে
                candles = []
                for _, row in df_limit.iterrows():
                    t_val = int(row['t'])
                    # মিলি-সেকেন্ড বনাম সেকেন্ড টাইমস্ট্যাম্প ডিটেকশন
                ...
                # ক্যান্ডেল কন্টেইনার এবং S&R লেভেলের জেসন অবজেক্ট রিটার্ন
                return jsonify({
                    "candles": candles,
                    "tf_res": tf_res,
                    "tf_sup": tf_sup,
        ...
        print(f"Error serving ohlcv data from Supabase Cloud: {e}")
...
<head>
    ...
    <!-- Lightweight Charts স্ট্যাবল ৪.২.৩ ভার্সন লকিং স্ক্রিপ্ট -->
...
<div class="max-w-md mx-auto">
    ...
    <!-- নতুন মাল্টি-টাইমফ্রেম বিশ্লেষণ কার্ড (ট্যাবড ভিউ) -->
    <div class="card p-4 mb-4 text-[11px]">
...
<script>
    ...
    // টাইমফ্রেম ডায়নামিক প্যারামিটারসহ ক্যান্ডেল লোড করার লজিক
    async function loadCandles() {
        try {
            ...
            if (data && data.length > 0) {
                ...
                // ডাইনামিক লাইভ S&R লাইন এবং বায়/সেল ইন্ডিকেটর আঁকা
    ...
    // চার্টের ওপর নির্দিষ্ট টাইমফ্রেমের সাপোর্ট-রেজিস্ট্যান্স এবং বায়/সেল ইন্ডিকেটর আঁকার লজিক
    function updateMarkersAndLines(tf_res, tf_sup, tf_name) {
        ...
        // ১. চার্ট উইন্ডোতে সিলেক্টেড টাইমফ্রেমের ডায়নামিক সাপোর্ট-রেজিস্ট্যান্স লাইন আঁকা
        if (tf_res > 0) {
            active_price_lines.push(candleSeries.createPriceLine({
                price: tf_res, 
                color: '#ef4444', // রেজিস্ট্যান্সের জন্য লাল কালার
                lineWidth: 1.5, 
                lineStyle: 1, 
        ...
        if (tf_sup > 0) {
            active_price_lines.push(candleSeries.createPriceLine({
                price: tf_sup, 
                color: '#10b981', // সাপোর্টের জন্য সবুজ কালার
                lineWidth: 1.5, 
                lineStyle: 1, 
                axisLabelVisible: true, 
                title: tf_name + ' Sup'
        ...
        // ২. রেফারেন্সের জন্য ম্যাক্রো ডেইলি হাই-লো (PDH, PDL) বর্ডার আঁকা
        if (d.pdh > 0) {
            active_price_lines.push(candleSeries.createPriceLine({
        ...
        // ৩. লাইভ ট্রেড পজিশন অ্যাক্টিভ থাকলে Entry, SL, TP লেভেল আঁকা
        if (d.in_position) {
            if (d.entry_price > 0) {
                active_price_lines.push(candleSeries.createPriceLine({
        ...
        // ৪. চার্টের সিলেক্টেড ক্যান্ডেল ইন্টারভ্যাল অনুযায়ী বায়/সেল ইন্ডিকেটর আঁকা
        if (d.history && d.history.length > 0) {
            const markers = [];
            const sortedHistory = [...d.history].sort((a, b) => a.ts - b.ts);
            
            // সিলেক্টেড টাইমফ্রেম অনুযায়ী ক্যান্ডেল এলাইনমেন্ট সেকেন্ড ম্যাপ
            ...
            sortedHistory.forEach(h => {
                if (!h.ts) return;
                // বর্তমান টাইমফ্রেমের ক্যান্ডেলে মার্কার সিনক্রোনাইজেশন
    ...
    // বাটন ক্লিক হলে চার্ট এবং বিশ্লেষণ বক্স রি-রেন্ডার হবে
    function selectTF(tf) {
        active_tf = tf;
        renderActiveTF();
        loadCandles(); // সিলেক্টেড টাইমফ্রেমের ডায়নামিক ক্যান্ডেল চার্ট লোড হবে
    }
    ...
    function renderActiveTF() {
        ...
        // পুরোনো ক্যাশ ডাটাজনিত কারণে কোনো কি মিসিং থাকলে যাতে ফ্রিজ না হয় তার ফ্যালব্যাক অবজেক্ট
    ...
    async function update() {
        try {
            ...
            if (d.price > 0) {
                ...
                renderActiveTF();
                // রি-রেন্ডারিং ডিলে এড়াতে শুধু প্যারামিটার আপডেট করে মার্কার ও ট্রেইলিং আপডেট করা হচ্ছে
