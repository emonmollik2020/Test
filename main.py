    ...
    while True:
        try:
            ...
            cur.update({
                ...
                "analysis_3m": {"rsi": round(ta.momentum.rsi(df3['c']).fillna(0).iloc[-1],1), "macd": round(m,3), "signal": "BULL" if m > ms else "WAIT", "pats": detect_pats(df3)},
            ...
            if not in_pos and p > e20 and r1 < 65 and m > ms:
                ...
                if live_pnl >= 0.7 or live_pnl <= -1.0:
                    ...
                    cur.update({"total_pnl": round(p_val, 2), "win_rate": round((wins/total)*100, 1), "last_action": "SELL"})
...
<head>
    ...
    <title>Pro AI Dashboard</title><script src="https://cdn.tailwindcss.com"></script>
    <style>
        ...
        .p-tag { padding: 4px 12px; border-radius: 99px; font-size: 11px; font-weight: 700; display: inline-block; margin: 2px; }
    </style>
</head>
<body class="p-4">
<div class="max-w-md mx-auto">
    ...
    <div class="grid grid-cols-3 gap-3 mb-4 text-center">
        <div class="card p-3"><p class="text-[8px] font-bold text-slate-400 uppercase">Trades</p><p id="t" class="text-lg font-extrabold">0</p></div>
        <div class="card p-3"><p class="text-[8px] font-bold text-slate-400 uppercase">Win Rate</p><p id="w" class="text-lg font-extrabold text-emerald-500">0%</p></div>
        <div class="card p-3"><p class="text-[8px] font-bold text-slate-400 uppercase">Profit</p><p id="pnl" class="text-lg font-extrabold text-blue-600">$0.00</p></div>
    </div>

    <div class="card p-6 mb-4 text-center">
        <div class="flex justify-between items-center mb-4"><h2 id="pr" class="text-5xl font-extrabold tracking-tight text-slate-900">$0.00</h2><p class="text-[9px] font-bold text-slate-400">Balance: <span id="bl" class="text-slate-800">$100.00</span></p></div>
        <div id="pnl_display" class="hidden mb-4 p-4 bg-slate-900 rounded-2xl text-white"><p class="text-[8px] font-bold text-slate-400 uppercase mb-1">Live Profit</p><p id="live_pnl" class="text-3xl font-extrabold">0.00%</p></div>
        <div id="st" class="bg-orange-50 text-orange-600 py-2.5 rounded-xl text-[10px] font-bold border border-orange-100 uppercase tracking-wide">&#8987; লোড হচ্ছে...</div>
    </div>

    <div class="grid grid-cols-2 gap-3 mb-4">
        <div class="card p-4"><div class="flex justify-between items-center mb-3"><h3 class="font-bold text-[10px] text-slate-400 uppercase">1m Scan</h3><span id="s1" class="text-[8px] font-bold px-2 py-0.5 rounded-full border">WAIT</span></div><p class="text-[11px] font-bold">RSI: <b id="r1">0</b></p><div id="p1" class="mt-2 flex flex-wrap"></div></div>
        <div class="card p-4"><div class="flex justify-between items-center mb-3"><h3 class="font-bold text-[10px] text-slate-400 uppercase">3m Scan</h3><span id="s3" class="text-[8px] font-bold px-2 py-0.5 rounded-full border">WAIT</span></div><p class="text-[11px] font-bold">MACD: <b id="m3">0</b></p><div id="p3" class="mt-2 flex flex-wrap"></div></div>
    </div>

    <div class="card overflow-hidden h-48 mb-4"><iframe src="https://s.tradingview.com/widgetembed/?symbol=BITGET%3ASOLUSDT&interval=1&theme=light" width="100%" height="100%" frameborder="0"></iframe></div>

    <div class="card p-4 mb-6"><h3 class="text-[9px] font-bold text-slate-300 uppercase tracking-widest mb-3">&#128203; Live Log</h3><div id="lg" class="space-y-2"></div></div>
    <p class="text-center text-[8px] font-bold text-slate-300 uppercase pb-6">Sync: <span id="ck">--:--:--</span></p>
</div>
    ...
    async function update() {
        try {
            ...
            if(d.price > 0) {
                ...
                document.getElementById('p1').innerHTML = d.analysis_1m.pats.map(p => `<span class="p-tag ${p.t === 'bull' ? 'tag-bull' : 'tag-bear'}">${p.n}</span>`).join('');
                document.getElementById('p3').innerHTML = d.analysis_3m.pats.map(p => `<span class="p-tag ${p.t === 'bull' ? 'tag-bull' : 'tag-bear'}">${p.n}</span>`).join('');
