...
def load_state():
    if not os.path.exists(STATE_FILE):
        return {
            ...
            "analysis_1m": {"rsi": 0, "ema20": 0, "ema50": 0, "bbu": 0, "bbl": 0, "sig": "অপেক্ষা", "pats": []},
            "analysis_3m": {"rsi": 0, "macd": 0, "ms": 0, "mh": 0, "sk": 0, "sd": 0, "sig": "অপেক্ষা", "pats": []},
...
def detect_pats(df):
    ...
    if body/full > 0.85 and c > o: p.append({"n": "বুলিশ মারুবোজু 💪", "t": "bull"})
    if body <= (full * 0.1): p.append({"n": "ডোজি ⚖️", "t": "neut"})
    if pc < po and c > o and c >= po and o <= pc: p.append({"n": "বুলিশ এনগালফিং 📈", "t": "bull"})
    if body > 0 and (min(c,o)-l) >= 1.8*body: p.append({"n": "হ্যামার 🔨", "t": "bull"})
    return p
...
def bot_engine():
    ...
    while True:
        try:
            ...
            s3_txt = "বুলিশ ✅" if m_obj.macd().iloc[-1] > m_obj.macd_signal().iloc[-1] else "বেয়ারিশ ❌"
            ...
            cur.update({
                ...
                "analysis_3m": {
                    ...
                    "sd": round(st_obj.stochrsi_d().iloc[-1]*100,1), "sig": s3_txt, "pats": detect_pats(df3)
                }
            })
            ...
            if not in_pos:
                ...
                cur["wait_reason"] = "ট্রেড লাইভ আছে (IN POSITION)"
                if live_pnl >= 0.7 or live_pnl <= -1.0:
                    ...
                    cur.update({"balance": round(100.0 + total_pnl_acc, 2), "total_pnl": round(total_pnl_acc, 2), "win_rate": round((wins/total)*100, 1), "best": round(max(pnl_history), 2), "worst": round(min(pnl_history), 2), "last_action": "SELL"})
...
<head>
    ...
    <style>
        ...
        .tag-bull { background: #dcfce7; color: #166534; padding: 2px 8px; border-radius: 99px; font-size: 10px; font-weight: 700; border: 1px solid #dcfce7; }
        .tag-bear { background: #fee2e2; color: #991b1b; padding: 2px 8px; border-radius: 99px; font-size: 10px; font-weight: 700; border: 1px solid #fee2e2; }
        .tag-neut { background: #f1f5f9; color: #475569; padding: 2px 8px; border-radius: 99px; font-size: 10px; font-weight: 700; border: 1px solid #e2e8f0; }
    </style>
</head>
...
<div class="max-w-md mx-auto">
    ...
    <div class="card p-6 mb-4">
        <div class="flex justify-between items-center mb-4">
            ...
            <div class="text-right text-[10px] text-slate-400">ব্যালেন্স: <b id="bl">$100.00</b><br>P&L: <b id="lpnl" class="text-green-600">+$0.00</b></div>
        </div>
    ...
    <div class="card p-4 mb-4 text-[11px]">
        <div class="flex justify-between mb-3 items-center"><h3 class="font-bold text-slate-700 text-xs">&#128202; 1 মিনিট বিশ্লেষণ</h3><span id="s1" class="font-bold px-2 py-0.5 rounded bg-slate-100">অপেক্ষা</span></div>
    ...
    <!-- 3m Box -->
    <div class="card p-4 mb-4 text-[11px]">
        <div class="flex justify-between mb-3 items-center"><h3 class="font-bold text-slate-700 text-xs">&#128202; 3 মিনিট বিশ্লেষণ</h3><span id="s3" class="font-bold px-2 py-0.5 rounded bg-slate-100">অপেক্ষা</span></div>
        <div class="grid grid-cols-2 gap-y-1.5">
    ...
    async function update() {
        try {
            ...
            if(d.price > 0) {
                ...
                s1.className = d.analysis_1m.sig.includes('বুলিশ') ? 'font-bold px-2 py-0.5 rounded bg-green-100 text-green-600' : 'font-bold px-2 py-0.5 rounded bg-red-100 text-red-600';
                ...
                const tag = (p) => `<span class="${p.t === 'bull' ? 'tag-bull' : (p.t === 'bear' ? 'tag-bear' : 'tag-neut')}">${p.n}</span>`;
