...
def bootstrap_or_backfill_sol():
    ...
    # মেমোরি গ্লোবাল ক্যাশে নিশ্চিত রাইট (যাতে চার্ট সাথে সাথে লোড হতে পারে)
    ...
    while True:
        try:
            try:
                ...
                # লাইভ ডাটা মেমোরি ক্যাশে কপি করা হচ্ছে
                with GLOBAL_DF_LOCK:
...
def api_ohlcv():
    ...
    সরাসরি মেমোরি ক্যাশ GLOBAL_DF থেকে ডাটা পাঠায়, ফলে ফাইল মিসিংয়ের কোনো সুযোগ নেই।
    """
    global GLOBAL_DF
    try:
        with GLOBAL_DF_LOCK:
            if not GLOBAL_DF.empty:
                df_limit = GLOBAL_DF.tail(200) # শেষ ২০০ ক্যান্ডেল নেওয়া হচ্ছে
                candles = []
...
# =====================================================================
# SECTION 6: ড্যাশবোর্ড UI টেমপ্লেট (সংশোধিত চার্ট লোডার সহ)
# =====================================================================
UI = """
<!DOCTYPE html>
<html lang="bn">
<head>
    ...
    <!-- standalone Lightweight Charts library -->
...
<div class="max-w-md mx-auto">
    ...
    <!-- ইন্টারেক্টিভ চার্ট উইন্ডো (স্থায়ীভাবে সাইজ এবং ডাইনামিক রেন্ডারিং সমস্যার সমাধান) -->
...
<script>
    ...
    // চার্ট ইনিশিয়ালাইজেশন মেথড
    function initChart() {
        const container = document.getElementById('lightweight_chart');
        if (!container) return;

        // কন্টেইনার স্টাইলিং এনশিওর করা
        container.style.width = '100%';
        container.style.height = '100%';

        // জিরো উইথ প্রবলেম এড়াতে উইন্ডো সাইজের সাথে কন্টেইনার ডাবল চেক করা হলো
        ...
        // ইনিশিয়াল ক্যান্ডেল লোড
        loadCandles();

        // মোবাইল বা ফাস্ট লোডের জন্য ১০০ মিলি-সেকেন্ড পর রিসাইজ এবং ফিট করে নেওয়া হচ্ছে
        setTimeout(() => {
            chart.resize(container.clientWidth || window.innerWidth || 340, 256);
    ...
    async function loadCandles() {
        try {
            ...
            if (data && data.length > 0) {
                ...
                chart.timeScale().fitContent(); // সব ক্যান্ডেল অটো-ফিট করার প্রফেশনাল মেথড
                if (global_data) {
    ...
    // সাপোর্ট-রেজিস্ট্যান্স এবং বায়/সেল ইন্ডিকেটর আঁকার লজিক
    function updateMarkersAndLines() {
        ...
        // ২. কাস্টম সাপোর্ট ও রেজিস্ট্যান্স জোন লাইন তৈরি
        if (d.pdh > 0) {
