...
def get_advanced_pats(df):
    """
    রানিং ক্যান্ডেলস্টিক বাদ দিয়ে সদ্য ক্লোজ হওয়া সম্পূর্ণ ক্যান্ডেলের ওপর ভিত্তি করে প্যাটার্ন ডিটেকশন।
    """
    p = []
    if len(df) < 4:
        return p
        
    # iloc[-2] হলো সদ্য ক্লোজ হওয়া ক্যান্ডেল, iloc[-3] হলো তার আগের ক্যান্ডেল
    c1, c2 = df.iloc[-2], df.iloc[-3]
    ...
    if l1 >= 2.0 * b1 and u1 <= 0.15 * t1 and b1 > 0: 
        p.append({"n": "হ্যামার 🔨", "t": "bull"})
    if u1 >= 2.0 * b1 and l1 <= 0.15 * t1 and b1 > 0: 
...
def bootstrap_or_backfill_sol():
    ...
    # মেমোরি গ্লোবাল ক্যাশে নিশ্চিত রাইট
    ...
    while True:
        try:
            ...
            if in_pos:
                ...
                cur["wait_reason"] = "سুইং এন্ট্রি প্যাটার্ন খুঁজছে..."
...
<script>
    ...
    // চার্টে আঁকা ডাইনামিক লাইনের ক্যাশ মেমোরি ভ্যারিয়েবল
    let active_tf_res = 0;
    let active_tf_sup = 0;
    let active_tf_name = '';
    ...
    async function loadCandles() {
        try {
            ...
            if (data && data.length > 0) {
                ...
                // কাস্টম S&R ক্যাশ ভ্যারিয়েবলে স্টোর করা হলো যাতে ৫ সেকেন্ড পর গ্লোবাল আপডেটে উধাও না হয়
    ...
    function updateMarkersAndLines(tf_res, tf_sup, tf_name) {
        ...
        // ১. চার্ট উইন্ডোতে সিলেক্টেড টাইমফ্রেমের ডাইনামিক সাপোর্ট-রেজিস্ট্যান্স লাইন আঁকা (মুছে যাওয়ার সমস্যা স্থায়ীভাবে সমাধান)
        if (tf_res > 0) {
        ...
        // ২. লাইভ ট্রেড পজিশন অ্যাক্টিভ থাকলে Entry, SL, TP লেভেল আঁকা
        if (d.in_position) {
            if (d.entry_price > 0) {
                active_price_lines.push(candleSeries.createPriceLine({
        ...
        // ৩. চার্টের সিলেক্টেড ক্যান্ডেল ইন্টারভ্যাল অনুযায়ী বায়/সেল ইন্ডিকেটর আঁকা
    ...
    async function update() {
        try {
            ...
            if (d.price > 0) {
                ...
                renderActiveTF();
                // রিফ্রেশ টাইমে গ্লোবাল ক্যাশ ভ্যারিয়েবল দিয়ে লাইন ধরে রাখা হলো
                updateMarkersAndLines(active_tf_res, active_tf_sup, active_tf_name);
