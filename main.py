...
# =====================================================================
import ccxt
import ccxt.pro as ccxtpro # ওয়েবসেকট সমর্থিত CCXT সংস্করণ
import pandas as pd
import ta
import time
import threading
import asyncio # আসিঙ্ক্রোনাস অপারেশনের জন্য
import json
import os
import math
...
# ইন-মেমোরি গ্লোবাল ডাটাফ্রেম ক্যাশ
...
# ওয়েবসেকট ক্লায়েন্ট ডিক্লারেশন
...
# =====================================================================
# SECTION 3.5: টাইম এসিটিমেটর হেল্পার
# =====================================================================
...
def merge_websocket_candles_to_global(new_candles_list):
    """ওয়েবসেকট থেকে আসা লাইভ ক্যান্ডেলগুলোকে ৫০০০ হিস্ট্রি ক্যান্ডেলের সাথে মার্জ করে।"""
    global GLOBAL_DF
    ...
    with GLOBAL_DF_LOCK:
        if GLOBAL_DF.empty:
            ...
            # ডুপ্লিকেট টাইমস্ট্যাম্প বাদ দিয়ে সর্বশেষ লাইভ ক্যান্ডেলটি ক্যাশে আপডেট রাখে
...
async def sol_websocket_task():
    """SOL/USDT লাইভ ওএইচএলসিভি ডেটা অনবরত স্ট্রীম করার আসিঙ্ক্রোনাস টাস্ক"""
    print("Starting SOL/USDT 15m WebSocket Stream...")
    while True:
        try:
            # বিটগেট থেকে ১৫ মিনিটের লাইভ মোমবাতি পর্যবেক্ষণ
...
async def btc_websocket_task():
    """BTC/USDT লাইভ ওএইচএলসিভি ডেটা অনবরত স্ট্রীম করার আসিঙ্ক্রোনাস টাস্ক"""
...
async def async_bot_engine():
    ...
    # সিঙ্ক্রোনাস ডাটাবেজ টেবিল ক্রিয়েশন নন-ব্লকিং থ্রেডে পাঠানো হলো
    ...
    # ওয়েবসেকট রিয়েল-টাইম টাস্কগুলো ব্যাকগ্রাউন্ডে স্টার্ট করা হলো
    asyncio.create_task(sol_websocket_task())
    asyncio.create_task(btc_websocket_task())
    
    # প্রথমবার কানেক্ট হতে ৩ সেকেন্ড সময় দিন
    await asyncio.sleep(3)

    while True:
        try:
            # বিটগেট থেকে বারবার এপিআই কল করার প্রয়োজন নেই, আমরা লোকাল রিয়েল-টাইম ক্যাশ কপি নেব
            with GLOBAL_DF_LOCK:
                df15 = GLOBAL_DF.copy()
            
            if df15.empty:
                cur = await asyncio.to_thread(load_state)
                cur["wait_reason"] = "ওয়েবসকেট সংযোগের জন্য অপেক্ষা করছে... 🛑"
            ...
            # ডাটাবেজ এবং স্টেট সেভ আসিঙ্ক্রোনাসলি রান হবে
            await asyncio.to_thread(save_state, cur)
            
        except Exception as e:
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Async Bot Engine Warning: {e}")
            
        # রিয়েল-টাইম মেমোরি ক্যাশ থেকে প্রতি ১০ সেকেন্ডে একবার হিসাব এবং রুলস চেক হবে
        await asyncio.sleep(10)


def start_asyncio_loop():
    """আলাদা ব্যাকগ্রাউন্ড থ্রেডে আসিঙ্ক্রোনাস লুপ চালানোর হেল্পার"""
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(async_bot_engine())


# Background Thread-এ আসিঙ্ক্রোনাস লুপ স্টার্ট করা হলো
...
def api_ohlcv():
    ...
    try:
        # Flask থ্রেড সেফটি বজায় রেখে গ্লোবাল ক্যাশ ডেটাফ্রেম রিড করবে
        with GLOBAL_DF_LOCK:
            df = GLOBAL_DF.copy()
        ...
        print(f"Error serving ohlcv data from Memory/DB: {e}")
...
# =====================================================================
# SECTION 6: ড্যাশবোর্ড UI টেমপ্লেট (অপরিবর্তিত রাখা হয়েছে)
# =====================================================================
UI = """
<!DOCTYPE html>
<html lang="bn">
