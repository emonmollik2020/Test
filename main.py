...
def bot_engine():
    ...
    while True:
        try:
            ...
            # ১৫-মিনিটের ডেটা মেমোরিতে ইনডেক্স করে নিয়ে অন্যান্য টাইমফ্রেমে কনভার্ট করা
            df15_temp = df15.copy()
            df15_temp['dt'] = pd.to_datetime(df15_temp['t'], unit='ms')
            df15_temp.set_index('dt', inplace=True)
            
            # নতুন টাইমফ্রেম জেনারেশন (রিস্যাম্পলিং)
            ...
            # সব টাইমফ্রেমের অ্যানালাইসিস অবজেক্ট জেনারেট করা
            analysis_15m = {
...
def api_ohlcv():
    """
    লাইটওয়েট চার্টস-এর জন্য ওএইচএলসিভি ক্যান্ডেল ডাটা পাঠানোর নতুন এন্ডপয়েন্ট।
    এটি হিস্ট্রি থেকে শেষ ২০০টি ক্যান্ডেল দ্রুত লোড করবে।
    """
    try:
        if os.path.exists(HISTORY_FILE):
            df = pd.read_csv(HISTORY_FILE)
            df_limit = df.tail(200) # পারফরম্যান্স অপ্টিমাইজেশনের জন্য শেষ ২০০ ক্যান্ডেল নেওয়া হচ্ছে
            candles = []
            for _, row in df_limit.iterrows():
                candles.append({
                    "time": int(row['t'] / 1000), # মিলি-সেকেন্ডকে সেকেন্ডে কনভার্ট করা হয়েছে
                    "open": float(row['o']),
                    "high": float(row['h']),
                    "low": float(row['l']),
                    "close": float(row['c'])
...
# =====================================================================
# SECTION 6: ড্যাশবোর্ড UI টেমপ্লেট (ডাইনামিক TradingView চার্ট সহ)
# =====================================================================
UI = """
<!DOCTYPE html>
<html lang="bn">
...
<div class="max-w-md mx-auto">
    ...
    <!-- ইন্টারেক্টিভ চার্ট উইন্ডো (নতুন রিয়েল-টাইম ক্যান্ডেল ও সাপোর্ট-রেজিস্ট্যান্স লাইন) -->
...
<script>
    ...
    // লাইটওয়েট চার্টস ভ্যারিয়েবলস
    let chart = null;
    let candleSeries = null;
    let active_price_lines = []; // চার্টে আঁকা সাপোর্ট-রেজিস্ট্যান্স লেভেল ট্র্যাকিং

    // ডাইনামিক চার্ট ইনিশিয়ালাইজেশন মেথড
    function initChart() {
        ...
        // উইন্ডো সাইজ পরিবর্তন হলে চার্ট অটোমেটিক অ্যাডজাস্ট হবে
        window.addEventListener('resize', () => {
            chart.resize(container.clientWidth, 256);
        });

        // প্রথমবার হিস্ট্রি ক্যান্ডেল লোড
        loadCandles();
    }

    // ব্যাকএন্ড API থেকে ওএইচএলসিভি ডাটা লোড ও আপডেট করার মেথড
    async function loadCandles() {
        try {
            ...
            if (data && data.length > 0) {
                ...
                // চার্ট ক্যান্ডেল আপডেটের পর মার্কার ও সাপোর্ট লাইন ড্র করা
                if (global_data) {
    ...
    // চার্টের ওপর সাপোর্ট-রেজিস্ট্যান্স লাইন এবং বায়/সেল ইন্ডিকেটর আঁকার লজিক
    function updateMarkersAndLines() {
        if (!global_data || !candleSeries) return;
        const d = global_data;

        // ১. আগের সব পুরাতন লাইনগুলো রিমুভ করা
        active_price_lines.forEach(line => candleSeries.removePriceLine(line));
        active_price_lines = [];

        // ২. কাস্টম সাপোর্ট ও রেজিস্ট্যান্স জোন লাইন তৈরি (PDH, PDL, H4 Support/Resistance)
        if (d.pdh > 0) {
        ...
        // ৪. চার্টের ক্যান্ডেলের ওপর বায়/সেল তীর চিহ্ন (Trade Markers) ড্র করা
        if (d.history && d.history.length > 0) {
            const markers = [];
            // হিস্ট্রি ক্রনোলজিক্যাল অর্ডারে সাজানো
            const sortedHistory = [...d.history].sort((a, b) => a.ts - b.ts);
            
            sortedHistory.forEach(h => {
                if (!h.ts) return;
                // ১৫-মিনিটের মোমবাতির শুরুর সেকেন্ডে এলাইনমেন্ট করা
    ...
    async function update() {
        try {
            ...
            if (d.price > 0) {
                ...
                // ডাইনামিক সিলেক্টেড টাইমফ্রেম রেন্ডার করা
                renderActiveTF();

                // লাইভ চার্টের মার্কার ও প্রাইস লাইন আপডেট করা
                updateMarkersAndLines();
    ...
    // প্রতি ১৫ সেকেন্ডে নতুন ক্যান্ডেলস্টিক ডাটা রিফ্রেশ হবে
    setInterval(loadCandles, 15000); 

    // প্রতি ৫ সেকেন্ডে সাধারণ ডাটা আপডেট হবে
    setInterval(update, 5000); 
    
    // উইন্ডো লোড হওয়ার সাথে সাথে চার্ট ইনিশিয়ালাইজেশন
    window.onload = () => {
        initChart();
        update();
    };
