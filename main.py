...
# ইন-মেমোরি গ্লোবাল ডাটাফ্রেম ক্যাশ (বটের ব্যবহারের জন্য)
...
def api_ohlcv():
    """
    এক্সচেঞ্জে কোনো এক্সটার্নাল রিকোয়েস্ট না পাঠিয়ে সরাসরি Supabase ডাটাবেজ থেকে 
    ডাটা এনে চার্ট তৈরি করার সম্পূর্ণ অফলাইন-সেফ ও ডাইনামিক টাইমস্ট্যাম্প বিশিষ্ট এন্ডপয়েন্ট।
    """
    try:
        if db_enabled:
            ...
            df = pd.read_sql("SELECT * FROM sol_15m_history ORDER BY t ASC", engine)
            if df is not None and not df.empty:
                # সোর্টিং এনশিওর করে শেষ ২০০ ক্যান্ডেল সিলেক্ট করা হচ্ছে
                df_limit = df.dropna(subset=['t','o','h','l','c']).sort_values('t').tail(200)
                candles = []
                for _, row in df_limit.iterrows():
                    t_val = int(row['t'])
                    # ডাইনামিক চেক: মিলি-সেকেন্ড (১৩ ডিজিট) বনাম সেকেন্ড (১০ ডিজিট) টাইমস্ট্যাম্প ডিটেকশন
    ...
    function initChart() {
        ...
        chart = LightweightCharts.createChart(container, {
            ...
            layout: {
                ...
                attributionLogo: false // TradingView ওয়াটারমার্ক লোগো হাইড করা হলো
            },
            grid: {
